<?php /* widget_cbnet_plugin_twitterprofile */
/**
 * Plugin Name: cbnet Twitter Profile Display
 * Plugin URI: http://www.chipbennett.net/wordpress/plugins/cbnet-twitter-profile-display/
 * Description: A widget that displays tweets from any user's Twitter profile.
 * Version: 1.0
 * Author: chipbennett
 ...
 * @since 1.0
 */
add_action( 'admin_notices', 'cbnet_twitter_profile_admin_notice' );

function cbnet_twitter_profile_admin_notice() {
	if ( ! file_exists( CBNETTWITTERWIDGETPATH ) ) {
		echo "<div class='updated' style='background-color:#f66;'><p>" . sprintf( 'The cbnet Twitter Profile Display Plugin is obsolete and no longer maintained. <strong>Please <a href="%s">install the cbnet Twitter Widget Plugin</a> in order to continue to receive updates and support.</strong>', cbnet_twitter_plugin_install_path() ) . "</p></div>";
 ...
 * @since 0.1
 */
function widget_cbnet_plugin_twitterprofile_load_widgets() {
	register_widget( 'widget_cbnet_plugin_twitterprofile' );
}

/**
 * widget_cbnet_plugin_twitterprofile class.
...
class widget_cbnet_plugin_twitterprofile extends WP_Widget {

    function widget_cbnet_plugin_twitterprofile() {
        $widget_ops = array('classname' => 'widget-cbnet-plugin-twitterprofile', 'description' => 'cbnet plugin widget to display Twitter profile tweets' );
        $this->WP_Widget('cbnet_plugin_twitterprofile', 'cbnet Twitter Profile', $widget_ops);
    }

    function widget( $args, $instance ) {
        extract($args);
        $title = apply_filters('widget_title', empty($instance['title']) ? 'Twitter Profile' : $instance['title']);
...
<?php
    ...
    function form( $instance ) {
		$defaults = array( 'title' => 'Twitter Profile', 'twitteruserid' => 'TwitterUserID', 'shellbg' => '#cccccc', 'shellcolor' => '#ffffff', 'tweetbg' => '#ffffff', 'tweetcolor' => '#444444', 'tweetlink' => '#5588aa', 'scrollbar' => 'true', 'loop' => 'false', 'live' => 'true', 'hashtags' => 'true', 'timestamp' => 'true', 'avatars' => 'true', 'behavior' => 'all', 'interval' => '6000', 'rpp' => '30', 'widthauto' => 'false', 'width' => '150', 'height' => '300' );
